	...
	function xuanxiangka1(btn11,con11,hongtiao11){
		const btn1=$(btn11);
		const con1=$(con11);
		const hongtiao1=$(hongtiao11)[0];
		for(let i=0;i<btn1.length;i++){   //for里let不能改成var
			...
			for(let j=0;j<con1.length;j++){
				...
				btn1[j].style.fontWeight="normal";
			}
			con1[i].style.display="block";
			btn1[i].style.fontWeight="bold";
			animate(hongtiao1,{left:btn1[i].offsetLeft},50)
			}
		}	
	}
...
function zjlunbo(pic,bigbannerBox,lunbodian,left,right,colorArr,tuBigZ,liactivebgColor,lunboTime,tuZ,lisColor){
			...
			const banner=document.querySelectorAll(bigbannerBox);
			...
			function move(type="l"){
				...
				for(let i=0;i<tu.length;i++){
					...
					banner[i].style.zIndex=tuZ;
					li[i].style.background=lisColor;
					// banner.style.background=color[i];

				}
				tu[num].style.zIndex=tuBigZ;
				banner[num].style.zIndex=tuBigZ;
				li[num].style.background=liactivebgColor;
				// banner.style.background=color[num];
			}
}
...
// 楼层跳转 按需加载******************1
	...
	// donghuaTime  动画时间
	// num  导航出现的高度
function louceng(anniu,Section,anniukuai,donghuaTime,num){
	const btn=$(anniu);
